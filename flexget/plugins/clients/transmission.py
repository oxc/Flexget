...
class PluginTransmissionClean(TransmissionBase):
    ...
    def on_task_exit(self, task, config):
        ...
        tracker_mode_exclude = (config['tracker_mode'].tolower() == 'exclude') if 'tracker_mode' in config else False
        ...
        for torrent in self.client.get_torrents():
            ...
            is_tracker_matching = not tracker_re or (tracker_mode_exclude != any(tracker_re.search(host) for host in tracker_hosts))
